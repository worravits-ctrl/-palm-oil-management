...
def create_app():
    ...
    def index():
        if current_user.is_authenticated:
            ...
            total_income = db.session.scalar(db.select(db.func.sum(HarvestIncome.net_amount)))
            total_expense = db.session.scalar(db.select(db.func.sum(FertilizerRecord.total_amount)))
    ...
    @login_required
    def harvest_list():
        # Join with Palm to get palm code
