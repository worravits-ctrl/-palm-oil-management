...
# ฟังก์ชันแปลงวันที่เป็นพุทธศั            # Generate final summary with AI
            data_summary = f"พบข้อมูล: {len(rows)} รายการ"
            if rows:
                summary_prompt = f"""
...
"""
    ...
    if sql and allow_sql(sql):
        try:
            ...
            # วิเคราะห์ข้อมูลพิเศษสำหรับตารางรายได้การตัดปาล์ม
            data_insights = ""
            if "harvest_income" in sql.lower() and len(rows) > 0:
                # คำนวณข้อมูลเสริมสำหรับการตัดปาล์ม
                if any(col in ['net_amount', 'total_weight_kg', 'price_per_kg'] for col in columns):
                    try:
                        insights_res = db.session.execute(db.text("""
                            SELECT 
                            ...
                            FROM harvest_income
                        """))
                        insights = insights_res.fetchone()
                        if insights:
                            data_insights = f"\n\n📊 สรุปข้อมูลการตัดปาล์ม:\n"
                            data_insights += f"• จำนวนครั้งที่ขาย: {insights[0]} ครั้ง\n"
                            data_insights += f"• รายได้รวม: {insights[1]:,.2f} บาท\n"
                            data_insights += f"• รายได้เฉลี่ย: {insights[2]:,.2f} บาท/ครั้ง\n"
                            data_insights += f"• น้ำหนักรวม: {insights[3]:,.2f} กก.\n"
                            data_insights += f"• ราคาเฉลี่ย: {insights[4]:,.2f} บาท/กก."
                    except Exception:
                        pass
            
            # Generate final summary with AI
            data_summary = f"ข้อมูลที่ได้: {len(rows)} แถว{data_insights}"
            if rows:
                summary_prompt = f"""
ข้อมูลที่ได้จาก SQL: {sql}
คอลัมน์: {columns}
จำนวนแถว: {len(rows)}
ข้อมูล 5 แถวแรก: {rows[:5]}

{summary_hint}

กรุณาสรุปข้อมูลนี้เป็นภาษาไทยให้เข้าใจง่าย พร้อมตัวเลขสำคัญ และข้อเสนอแนะ (ถ้ามี)
"""
                try:
                ...
                # ตรวจสอบว่าเป็นคำถามเกี่ยวกับรายได้การตัดปาล์มไหม
                if "รายได้" in message or "ตัดปาล์ม" in message or "ขาย" in message or "harvest_income" in sql.lower():
                    final_answer = ("ยังไม่มีข้อมูลรายได้การตัดปาล์มในระบบ\n\n"
                                   "💡 **เริ่มต้นใช้งาน:**\n"
                                   "1. ไปที่เมนู 'รายได้การตัดปาล์ม'\n"
                                   "2. เพิ่มรายการขายปาล์มครั้งแรก\n"
                                   "3. กลับมาถาม AI เกี่ยวกับข้อมูลได้เลย\n\n"
                                   "🌴 **ตัวอย่างคำถามที่จะถามได้:**\n"
                                   "• 'รายได้สุทธิเดือนนี้เท่าไหร่?'\n"
                                   "• 'ราคาปาล์มเฉลี่ยเดือนที่แล้ว?'\n"
                                   "• 'วันไหนขายปาล์มได้เงินมากที่สุด?'")
                else:
